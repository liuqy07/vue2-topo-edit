...
export default (G6, vue) => {
  const imglist = []  //vue.$store.state.topo.imgurl
  G6.registerNode(
    'base-node',
    {
      getShapeStyle(cfg) {
      ...
      drawShape(cfg, group) {
        ...
        if (cfg.img) {
          const { width, height } = attrs
            ...
            // if (cfg.radio === 0 && cfg.type == 'img-cloud') {
            //   // 表示用户使用的层级的图片
            //   img = imglist.find((_item) => _item.level == cfg.name)?.value ?? require('@/assets/images/topo/ifitTopo_new/p_5.png')
            // } else {
            //   if (cfg.img.includes('/')) {
            //     try {
            //       img = require('@/assets/images/topo/ifitTopo_new/' + cfg.img.split('/')[cfg.img.split('/').length - 1])
            //     } catch {
            //       img = imglist.find((_item) => _item.level == cfg.name)?.value ?? require('@/assets/images/topo/ifitTopo_new/p_5.png')
            //     }
            //   } else {
            //    img = require('@/assets/images/topo/ifitTopo_new/' + cfg.img)
            //  }
            // }
